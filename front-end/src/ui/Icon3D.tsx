...
export function Icon3D({ icon: Icon, colorType = 'blue' }: Icon3DProps) {
  const colors = {
    ...
    slate:
      'from-slate-600 via-slate-700 to-slate-900 shadow-[inset_0px_-3px_8px_rgba(0,0,0,0.4),inset_0px_3px_8px_rgba(255,255,255,0.2),0_6px_15px_-3px_rgba(15,23,42,0.4)]',
  ...
  return (
    <div className="relative w-14 h-14 group perspective-1000 mb-6">
      ...
      <div
        className={`absolute inset-0 bg-gradient-to-br ${colors[colorType]} rounded-2xl transition-transform duration-300 group-hover:-translate-y-1.5 flex items-center justify-center overflow-hidden border border-white/20`}
      >
        ...
        <Icon
          size={26}
          className="text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)] relative z-10 transform transition-transform duration-300 group-hover:scale-110 group-hover:rotate-y-12"
          strokeWidth={2.5}
        />
      </div>
    </div>
