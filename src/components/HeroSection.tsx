...
export function HeroSection() {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    setError('');
    // TODO: integrar com serviço de e-mail (Resend / Mailchimp)
    console.info('[HeroSection] Lead capturado:', email);
    setSubmitted(true);
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-8 items-center">
          ...
          >
            ...
            <h1
              id="hero-heading"
              className="text-5xl xl:text-6xl font-bold tracking-tight text-slate-900 leading-[1.1] mb-6 lg:pr-12 relative z-30"
            >
            ...
            <p className="text-lg text-slate-600 mb-8 leading-relaxed max-w-xl">
              O PrazoAlert não monitora apenas seus prazos 24h por dia. Ele centraliza sua rotina,
              elimina o tempo perdido em portais lentos e traz organização inteligente para o seu
              dia a dia via WhatsApp.
            </p>

            {/* ── Lead capture form ── */}
            <div id="hero-form" className="flex flex-col gap-3 mt-8 max-w-lg">
              {submitted ? (
                ...
                <form
                  ...
                  noValidate
                  aria-label="Formulário de cadastro para teste grátis"
                >
                  <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    <button
                      type="submit"
                      className="btn-primary px-8 py-4 font-semibold flex items-center justify-center gap-2 transition-all whitespace-nowrap"
                    >
                      Testar Grátis <ArrowRight size={20} aria-hidden="true" />
                    </button>
                  </div>
                  {error && (
                    <p
                  ...
                  <div className="flex items-center flex-wrap gap-4 text-xs text-slate-500 font-medium mt-3 ml-1">
                    <span className="flex items-center gap-1">
                      <CheckCircle2 size={14} className="text-emerald-500" aria-hidden="true" /> 7 dias grátis
                    </span>
                    <span className="flex items-center gap-1">
                      <CheckCircle2 size={14} className="text-emerald-500" aria-hidden="true" /> Liberação imediata
                    </span>
                  </div>
                </form>
              )}
            ...
            <div className="mt-8 flex items-center gap-4 text-sm text-slate-500">
              <div className="flex -space-x-2" aria-hidden="true">
                {[
                  { src: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=64&h=64', alt: 'Usuário' },
                  { src: 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?auto=format&fit=crop&w=64&h=64', alt: 'Usuário' },
                  { src: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=64&h=64', alt: 'Usuário' },
                ].map(({ src, alt }, i) => (
                  <img
                    key={i}
                    src={src}
          ...
          >
            ...
            >
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 shrink-0" aria-hidden="true">
                <Clock size={20} />
              </div>
              <div>
                <p className="text-xs sm:text-sm font-medium text-slate-500">Tempo recuperado</p>
                <p className="text-lg sm:text-xl font-bold text-slate-900">+40h / mês</p>
              </div>
            </motion.div>

            {/* Dashboard mockup */}
            <div className="absolute top-10 sm:top-12 lg:top-4 right-0 w-[95%] sm:w-[85%] lg:w-[500px] max-w-full glass-dark p-3 sm:p-5 z-10 rounded-[1.5rem] sm:rounded-[2rem] shadow-2xl transform lg:translate-x-12 lg:-translate-y-4" aria-hidden="true">
              <div className="w-full h-full bg-slate-900/50 rounded-xl sm:rounded-2xl border border-slate-700/50 p-4 flex flex-col gap-4">
                ...
                <div className="flex gap-4 h-[200px] sm:h-[240px]">
                  ...
                  <div className="w-1/3 flex flex-col gap-3">
                    {[
                      { label: 'w-2/3', color: 'bg-white' },
                      { label: 'w-1/2', color: 'bg-blue-400' },
                      { label: 'w-3/4', color: 'bg-emerald-400', border: 'border-emerald-900/50 bg-emerald-900/20' },
                    ].map((item, i) => (
                      <div
                        key={i}
                        className={`flex-1 border border-slate-700/50 rounded-xl bg-slate-800/30 p-3 flex flex-col justify-center ${
                          item.border ?? ''
                        }`}
                      >
                        <div className="w-full h-2 bg-slate-600 rounded mb-2" />
                        <div className={`${item.label} h-4 ${item.color} rounded`} />
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <motion.div
              ...
              aria-label="Exemplo de notificação WhatsApp do PrazoAlert"
            >
              <div className="relative w-full h-full bg-[#E5DDD5] rounded-[2rem] overflow-hidden flex flex-col">
                {/* Notch */}
                <div className="absolute top-0 inset-x-0 h-6 flex justify-center z-50" aria-hidden="true">
                ...
                <div className="bg-[#075E54] pt-8 pb-3 px-4 flex items-center gap-3 shadow-md z-20">
                  <div className="w-9 h-9 rounded-full bg-white flex items-center justify-center shrink-0" aria-hidden="true">
            ...
            {/* Ambient glow */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 h-72 bg-blue-400/30 rounded-full blur-[80px] -z-10" aria-hidden="true" />
          </motion.div>
